...
def getweather():
    ...
    if request.method == "GET":
        ...
        city, node = request.args.get('city', ''), request.args.get('node', '')
        if node and node.upper() != "NODEMCU":
            return jsonify({"code":"error","data":"node参数值不正确！node's value error！"})
        weather_data = w.get_weather_data(city, node)
        return jsonify(weather_data) if inspect else jsonify({"code":"error","data":"参数错误！Parameter error！"})
